...
pub fn update_tracers(mut commands:Commands,mut tracer_query:Query<(&mut BulletTracer,&mut Transform,Entity)>,time:Res<Time>)
{
    for (mut tracer,mut transform,entity) in tracer_query.iter_mut()
    {
        ...
        tracer.start_position = move_towards(tracer.start_position, tracer.end_position, time.delta_seconds()*50.);
