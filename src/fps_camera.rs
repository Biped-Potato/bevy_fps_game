use bevy::{input::mouse::MouseMotion, prelude::*, core::Zeroable};
...
pub fn move_camera(
    ...
    if cursor_lock_state.state {
        for (mut transform, mut camera) in camera_query.iter_mut() {
            ...
            camera.recoil_shake = move_towards(camera.recoil_shake,Vec3::ZERO,time.delta_seconds()*camera.camera_shake_readjustment_factor);
            let x_quat = Quat::from_axis_angle(Vec3::new(0., 1., 0.), camera.rotation.y-camera.recoil_shake.x);

            let y_quat = Quat::from_axis_angle(Vec3::new(1., 0., 0.), camera.rotation.x+camera.recoil_shake.y);
