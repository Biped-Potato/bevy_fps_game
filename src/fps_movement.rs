...
pub fn player_movement(time: Res<Time>,rapier_context: Res<RapierContext>,mut movement_query : Query<(&Transform,&mut Damping,&mut FPSCamera,&mut FPSMovement,&mut Velocity)>,key: Res<Input<KeyCode>>,)
{
    for (transform,mut damping,mut camera,mut movement,mut velocity) in movement_query.iter_mut()
    {
        ...
        let ray = Ray{origin:transform.translation-0.5,direction:Vec3::new(0.,-1.,0.)};
        ...
        velocity.linvel.x += direction.x*movement.acceleration*time.delta_seconds()*air_modifier;
        velocity.linvel.z += direction.y*movement.acceleration*time.delta_seconds()*air_modifier;
